...
class Line:
    ...
    def GetDelta(self):
        return (abs(self.ptTo.x - self.ptFrom.x), abs(self.ptTo.y - self.ptFrom.y))

    def IsStraight(self):
        return self.IsHoriz() or self.IsVert() or self.GetDelta()[0] == self.GetDelta()[1]

    def GetDirection(self):
        return Point(1 if self.ptFrom.x <= self.ptTo.x else -1, 1 if self.ptFrom.y <= self.ptTo.y else -1)
    ...
    def GetPoints(self):
        ...
        if self.IsHoriz():
            ...
            for i in range(self.GetDelta()[0] + 1):
                points.append(
                    Point(self.ptFrom.x + i * self.GetDirection().x, self.ptFrom.y + i * self.GetDirection().y)
                )
        return tuple(points)
