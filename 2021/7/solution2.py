...
class Swarm:
    def __init__(self, string) -> None:
        ...
        self.fuelPerDist = [0 for p in range(0, self.max)]
        for i in range(0, self.max):
            costOfPos = sum([j for j in range(0, i + 1)])
            self.fuelPerDist[i] = costOfPos
        print("input positions:", str(self.positions))
        print("input values:%d maxIndex:%d" % (len(inputPos), self.max))
    ...
    def FuelForPos(self, index):
        ...
        for i in range(0, self.max):
            ...
            costOfPos = self.fuelPerDist[distance]
            totalFuel = totalFuel + (costOfPos * self.positions[i])
        return totalFuel
